...
def parse_landmark_line(line: str):
    ...
    for pair in line.split(";"):
        ...
        if pair and not pair.startswith("/"):  # skip empty or trailing /id
            ...
            coords.append([x, y])  # JSON prefers lists, not tuples
    return coords
...
def create_mapping(landmark_file: Path, image_dir: Path, output_dir: Path):
    ...
    # --- Build mapping (filename → {path, landmarks}) ---
    mapping = {}
    for img, coords in zip(images, landmarks):
        mapping[img.name] = {
            "path": str(img.resolve()),   # absolute path for clarity
            "landmarks": coords
        }
